...
+ 首先进行顺向排序（从小到大），因为排序的时间复杂度是 O ( nlogn )  ，不用担心排序会影响时间
+ 然后固定两个字符 i 和 j，然后最后一个字符 k 在数组最后面开始向左遍历，计算下一次结果，如果下一次结果大于 0 ，则代表需要继续缩小 k 的值。
+ 如果下一次结果（k-1）不大于 0 了，则代表下一次结果可能是目标，则判断加起来的结果是否等于 0。
+ 这可以排除掉一定不符合规则的可能，**如果当前 k 的值不能大于 0 了，那么向左遍历的值（比当前 k 的值小）就一定不符合**。
